...
# LANGUAGE SYSTEM
# ============================================================
$ScriptName    = "win11_LTSC"
$LogDate       = Get-Date -Format "yyyy-MM-dd_HH-mm"
$LogDateHuman  = (Get-Date).ToString("g")
...
if (-not $Lang) {
    $Lang = @{
        ScriptName       = "win11_LTSC"
        ReportTitle      = "TITANIUM V8 - OPTIMIZATION REPORT (IoT LTSC)"
        ...
        FooterSendFile   = "In case of issues, send this file to your technician."
        FooterBackupPath = "Log backup:"
        SummaryTitle     = "=== TITANIUM V8 WIN11 LTSC - COMPLETED ==="
        SummaryDefender  = " -> Adobe/CAD Suite and Drivers: OPTIMIZED."
        SummaryBloatware = " -> SSD/NVMe: TRIM active, unnecessary writes eliminated."
        SummaryMSA       = " -> GPU/CPU/I/O Performance Engine: APPLIED."
        SummarySSD       = " -> Legacy Startup and OS Behavior: CLEANED."
        SummaryPerf      = " -> Report saved."
        SummaryReboot    = " -> System will reboot in 10 seconds."
        PopupTitle       = "Titanium V8 IoT LTSC - Completed"
        PopupBody        = "Optimization completed!`n`nReport saved to Desktop:`n{0}`n`nIn case of issues, send this file to your technician.`n`nThe PC will reboot in a few seconds."
    }
}
...
$LogDesktop   = "$_realUserDesktop\$_LogFileName"
...
║ $($Lang.LabelPS):          $_psVer
║ $($Lang.LabelRunAs):       $($Lang.LabelRunAsSystem)
...
"@
    ...
    Write-Host ("$($Lang.SummaryPerf) $LogDesktop") -ForegroundColor Cyan

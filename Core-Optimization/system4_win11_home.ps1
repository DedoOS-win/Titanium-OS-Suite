...
# ------------------------------------------------------------
# LOGGING SYSTEM
# User desktop logs + backup to C:\Windows\Logs\System4\
# Readable by non-technical users + technical support # ------------------------------------------------------------
...
$LogDesktop  = "$DesktopPath\System4_Log_$LogDate.txt"
...
$_runAs   = if ($isSystem) { "SYSTEM (PowerRun)" } else { "Amministratore" }
...
$header = @"
╔══════════════════════════════════════════════════════════════╗
║           TITANIUM V8 - OPTIMIZATION REPORT                ║
╠══════════════════════════════════════════════════════════════╣
║ Data:     $LogDateHuman
║ Computer: $_pcName
║ Utente:   $_user
...
Write-Host "[INIT] Creating a restore point..." -ForegroundColor Cyan
...
& {
    ...
    Write-Host "|  INFORMATION - Windows Search Function                   |" -ForegroundColor Cyan
    Write-Host "|                                                          |" -ForegroundColor Cyan
    Write-Host "|  The script is about to disable the Windows search box   |" -ForegroundColor Cyan
    Write-Host "|  and the connection to the Microsoft cloud.              |" -ForegroundColor Cyan
    Write-Host "|                                                          |" -ForegroundColor Cyan
    Write-Host "|  Searching for files on your PC will continue to work    |" -ForegroundColor Cyan
    Write-Host "|  via the EVERYTHING program (install or portable).       |" -ForegroundColor Cyan
    Write-Host "|                                                          |" -ForegroundColor Cyan
    Write-Host "|  Will be disabled:                                       |" -ForegroundColor Cyan
    Write-Host "|  - Bing search from the taskbar                          |" -ForegroundColor Cyan
    Write-Host "|  - Microsoft cloud suggestions                           |" -ForegroundColor Cyan
    Write-Host "|  - Search box in taskbar                                 |" -ForegroundColor Cyan
    Write-Host "|  - WebView2 Search (background process)                  |" -ForegroundColor Cyan
    Write-Host "|                                                          |" -ForegroundColor Cyan
    ...
    # --------------------------------------------------------
    & reg.exe add "HKLM\SOFTWARE\Policies\Microsoft\Windows\Windows Search" /v "DisableSearch"         /t REG_DWORD /d 1 /f 2>$null
    & reg.exe add "HKLM\SOFTWARE\Policies\Microsoft\Windows\Windows Search" /v "ConnectedSearchUseWeb" /t REG_DWORD /d 0 /f 2>$null
    & reg.exe add "HKLM\SOFTWARE\Policies\Microsoft\Windows\Windows Search" /v "AllowCloudSearch"      /t REG_DWORD /d 0 /f 2>$null
    & reg.exe add "HKLM\SOFTWARE\Policies\Microsoft\Windows\Windows Search" /v "AllowCortana"          /t REG_DWORD /d 0 /f 2>$null
    & reg.exe add "HKLM\SOFTWARE\Policies\Microsoft\Windows\Windows Search" /v "BingSearchEnabled"     /t REG_DWORD /d 0 /f 2>$null
    & reg.exe add "HKLM\SOFTWARE\Policies\Microsoft\Windows\Windows Search" /v "DisableWebSearch"      /t REG_DWORD /d 1 /f 2>$null
    & reg.exe add "HKCU\Software\Microsoft\Windows\CurrentVersion\Search"   /v "SearchboxTaskbarMode"  /t REG_DWORD /d 0 /f 2>$null
...
& {
    ...
    & reg.exe delete "HKCU\Software\Microsoft\Windows\CurrentVersion\Run" /v "Adobe Creative Cloud" /f 2>$null | Out-Null
    ...
    # --------------------------------------------------------
    # CONFIRM CONSOLE - Disabling Windows Search
    # Use Read-Host (compatible with SYSTEM context)
    # --------------------------------------------------------
    Write-Host "`n+----------------------------------------------------------+" -ForegroundColor Yellow
    Write-Host "|  WARNING - Windows Search (WSearch)                      |" -ForegroundColor Yellow
    Write-Host "|                                                          |" -ForegroundColor Yellow
    Write-Host "|  The SEARCH FOR YOUR FILES feature will be DISABLED.     |" -ForegroundColor Yellow
    Write-Host "|  After disabling it, you will need to use EVERYTHING     |" -ForegroundColor Yellow
    Write-Host "|  to search for files on your PC.                         |" -ForegroundColor Yellow
    Write-Host "|                                                          |" -ForegroundColor Yellow
    Write-Host "|  S = Disable WSearch (recommended with Everything)       |" -ForegroundColor Green
    Write-Host "|  N = Keep WSearch enabled                                |" -ForegroundColor Cyan
    Write-Host "+----------------------------------------------------------+" -ForegroundColor Yellow

    $risposta = Read-Host "`nDisable Windows Search? (S/N)"

    if ($risposta -match "^[Ss]$") {
        Stop-Service -Name "WSearch" -Force -ErrorAction SilentlyContinue
        Set-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Services\WSearch" -Name "Start" -Value 4 -Force
        Write-Host "   WSearch: DISABLED. Use Everything to search for files." -ForegroundColor Yellow
    } else {
        Set-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Services\WSearch" -Name "Start" -Value 3 -Force
        Write-Host "   WSearch: PRESERVATO su richiesta." -ForegroundColor Green
    }

    Write-Host "-> Adobe/Office/NVIDIA cleanup complete." -ForegroundColor Green
}
...
& {
    ...
    # NOTE: Energy plan left on BALANCED intentionally
    # Some machines have thermal issues with High Performance
    Write-Host "   Energy plan: BALANCED preserved (thermal management)." -ForegroundColor Gray
...
& {
    ...
    Write-Host "   PcaSvc (Program Compatibility): PRESERVED on Win11 Home." -ForegroundColor Gray
...
& {
    ...
    $DisableOneDrive = Ask-YesNoPopup `
        -Title "System4 Win11 Home - OneDrive" `
        -Message "Do you want to disable OneDrive? nIf you choose No, OneDrive remains operational."
    if ($DisableOneDrive) {
        Invoke-ZombiePurge -DisableOneDrive $true

        # Uninstall classic OneDrive (if present) - strong anti-reappearance
        if (Test-Path "$env:SystemRoot\SysWOW64\OneDriveSetup.exe") {
            Start-Process "$env:SystemRoot\SysWOW64\OneDriveSetup.exe" "/uninstall" -WindowStyle Hidden -Wait -ErrorAction SilentlyContinue
        } elseif (Test-Path "$env:SystemRoot\System32\OneDriveSetup.exe") {
            Start-Process "$env:SystemRoot\System32\OneDriveSetup.exe" "/uninstall" -WindowStyle Hidden -Wait -ErrorAction SilentlyContinue
        }
        Write-Host "   OneDrive: disabled upon user request." -ForegroundColor Gray
    } else {
        Invoke-ZombiePurge -DisableOneDrive $false
        Write-Host "   OneDrive: kept active upon user request." -ForegroundColor Gray
    }
...
reg.exe delete "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" /v "OneDrive" /f 2>$null
reg.exe delete "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" /v "OneDrive" /f 2>$null
reg.exe delete "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" /v "OneDriveSetup" /f 2>$null
reg.exe delete "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" /v "OneDriveSetup" /f 2>$null
reg.exe delete "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" /v "TeamsMachineInstaller" /f 2>$null
...
& {
    ...
    $LogResult   = if ($script:LogErrors -eq 0) { "COMPLETED SUCCESSFULLY" } else { "COMPLETED WITH ERRORS" }
...
╔══════════════════════════════════════════════════════════════╗
║  RISULTATO: $LogResult
║  Durata:    $DurMin minuti e $DurSec secondi
║  Operazioni OK: $script:LogBlocksOk   Avvisi: $script:LogWarnings   Errori: $script:LogErrors
╚══════════════════════════════════════════════════════════════╝

In case of problems, send this file to your technician.
File also saved in: C:\Windows\Logs\System4\
"@
    ...
    Write-Host "`n=== TITANIUM V8 WIN11 HOME 26100 COMPLETED ===" -ForegroundColor Green
    Write-Host " -> Defender/SmartScreen/Tamper Protection: DISABLED." -ForegroundColor White
    Write-Host " -> Bloatware Xbox/Teams/OneDrive/Recall: REMOVED." -ForegroundColor White
    Write-Host " -> Account Microsoft e Consumer Features: BLOCKED." -ForegroundColor White
    ...
    Write-Host " -> The system will reboot in 12 seconds." -ForegroundColor Yellow

    # Final popup readable even by non-technical people
    try {
        Add-Type -AssemblyName System.Windows.Forms -ErrorAction SilentlyContinue
        [System.Windows.Forms.MessageBox]::Show(
            "Optimization complete!`n`nA report file has been saved to the Desktop:`n$LogDesktop`n`nIf you have any problems, send this file to your technician.`n`nThe  PC will restart in a few seconds.",
            "Titanium V8 - Completed",
